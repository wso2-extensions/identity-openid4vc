...
package org.wso2.carbon.identity.openid4vc.presentation.core.listener;
...
public class VPIdPMgtListener extends AbstractIdentityProviderMgtListener {
    ...
    private static final Log LOG = LogFactory.getLog(VPIdPMgtListener.class);
    ...
    public boolean doPostAddIdP(IdentityProvider identityProvider, String tenantDomain)
        ...
        if (!isDigitalWalletConnection(identityProvider)) {
            return true;
        }
     ...
     */
    private boolean isDigitalWalletConnection(IdentityProvider identityProvider) {
    ...
    private void provisionEcdsaKeyPairIfAbsent(String tenantDomain) throws Exception {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Generating P-256 ECDSA keypair for tenant " + tenantDomain + ".");
        }
