...
public class VPSessionStore {
    ...
    /**
     * Serializes a {@link VPSession} to JSON, encrypting only the secret fields
     * ({@code nonce} and {@code ephemeralPrivateKeyJwk}) before storage.
     */
    private static byte[] toStoredBytes(VPSession session) throws CryptoException {
    ...
    /**
     * Deserializes a {@link VPSession} from stored JSON bytes, decrypting the secret fields.
     */
    private static VPSession fromStoredBytes(byte[] storedBytes) throws CryptoException {
        ...
        return GSON.fromJson(json, VPSession.class);
    }
}
