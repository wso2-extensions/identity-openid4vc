...
public class OpenID4VPIdentityProviderMgtListener extends AbstractIdentityProviderMgtListener {
    ...
    private boolean hasClaimChanges(PresentationDefinition definition, List<String> mappedIdpClaims) {
        ...
        for (PresentationDefinition.RequestedCredential credential : definition.getRequestedCredentials()) {
            List<ClaimConstraint> existingConstraints = credential != null ? credential.getClaims() : null;
            Set<String> existingClaimSet = new LinkedHashSet<>();
            if (existingConstraints != null) {
                for (ClaimConstraint cc : existingConstraints) {
                    if (cc != null && cc.getName() != null) {
                        existingClaimSet.add(cc.getName());
                    }
                }
            }
    ...
    private PresentationDefinition buildSyncedDefinition(PresentationDefinition definition,
        ...
        if (existingCredentials != null) {
            for (PresentationDefinition.RequestedCredential credential : existingCredentials) {
                ...
                List<ClaimConstraint> syncedClaims = new ArrayList<>();
                for (String claimName : mappedIdpClaims) {
                    ClaimConstraint cc = new ClaimConstraint(claimName);
                    cc.setMandatory(true);
                    syncedClaims.add(cc);
                }
                updatedCredential.setClaims(syncedClaims);
