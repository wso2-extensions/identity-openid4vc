...
package org.wso2.carbon.identity.openid4vc.config.management;
...
import org.wso2.carbon.identity.openid4vc.config.management.constant.VCConfigManagementConstants;
import org.wso2.carbon.identity.openid4vc.config.management.dao.VCConfigMgtDAO;
import org.wso2.carbon.identity.openid4vc.config.management.dao.impl.VCConfigMgtDAOImpl;
...
import org.wso2.carbon.identity.openid4vc.config.management.internal.VCConfigManagementServiceDataHolder;
...
import org.wso2.carbon.identity.openid4vc.config.management.util.VCConfigFilterUtil;
...
import static org.wso2.carbon.identity.openid4vc.config.management.constant.VCConfigManagementConstants.DEFAULT_SIGNING_ALGORITHM;
import static org.wso2.carbon.identity.openid4vc.config.management.constant.VCConfigManagementConstants.VC_DIALECT;
...
public class VCCredentialConfigManagerImpl implements VCCredentialConfigManager {
    ...
    public VCCredentialConfiguration add(VCCredentialConfiguration configuration, String tenantDomain)
        ...
        addVCResource(configuration, tenantDomain);
        try {
            return dao.add(configuration, tenantId);
        } catch (VCConfigMgtException e) {
            if (VCConfigManagementConstants.ErrorMessages.ERROR_CODE_TRANSACTION_ERROR.getCode().equals(e.getCode())) {
                // Rollback VC resource addition if DAO operation fails.
                deleteVCResource(configuration, tenantDomain);
            }
            throw e;
        }
    }
    ...
    public void delete(String id, String tenantDomain) throws VCConfigMgtException {
        ...
        int tenantId = IdentityTenantUtil.getTenantId(tenantDomain);
        VCCredentialConfiguration configuration = dao.get(id, tenantId);
        deleteVCResource(configuration, tenantDomain);
        try {
    ...
    /**
     * Add VC API resource for the given VC configuration.
     *
     * @param configuration VC credential configuration.
     * @param tenantDomain  Tenant domain.
     */
    private void addVCResource(VCCredentialConfiguration configuration, String tenantDomain) {
        try {

            List<Scope> scopes = new ArrayList<>();
            Scope.ScopeBuilder scopeBuilder = new Scope.ScopeBuilder()
                    .name(configuration.getIdentifier())
                    .displayName(configuration.getIdentifier())
                    .description(configuration.getIdentifier());
            ...
            LOG.error("Error while adding VC API resource for VC configuration: " + configuration.getIdentifier() +
                    " in tenant: " + tenantDomain, e);
        }
    }

    /**
     * Delete VC API resource for the given VC configuration.
     *
     * @param configuration VC credential configuration.
     * @param tenantDomain  Tenant domain.
     */
    private void deleteVCResource(VCCredentialConfiguration configuration, String tenantDomain) {
        try {
