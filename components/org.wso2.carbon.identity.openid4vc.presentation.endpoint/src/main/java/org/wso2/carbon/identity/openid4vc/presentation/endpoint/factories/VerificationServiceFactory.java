...
/**
 * Factory for retrieving the {@link VerificationService} OSGi service instance
 * from the Carbon runtime. Loaded once at WAR startup.
 */
public class VerificationServiceFactory {

    private static final VerificationService SERVICE;

    static {
        VerificationService service = (VerificationService) PrivilegedCarbonContext
                .getThreadLocalCarbonContext().getOSGiService(VerificationService.class, null);

        if (service == null) {
            throw new IllegalStateException("VerificationService is not available from OSGi context.");
        }
        SERVICE = service;
    }

    private VerificationServiceFactory() {
