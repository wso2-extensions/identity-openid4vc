<%-- ~ Copyright (c) 2025, WSO2 LLC. (http://www.wso2.com). ~ ~ WSO2 LLC. licenses this file to you under the Apache
    ...
    <%@ page contentType="text/html;charset=UTF-8" language="java" %>
        <%@ page import="java.util.UUID" %>
            <% // Get wallet authentication state (unique for this wallet auth attempt) String
                walletState=request.getParameter("walletState"); String
                sessionDataKey=request.getParameter("sessionDataKey"); // Get context path String
                contextPath=request.getContextPath(); if (contextPath==null) { contextPath="" ; } // Check for missing
                parameters (we'll show error in UI instead of redirecting) boolean hasError=false; String
                errorMessage="" ; if (walletState==null || walletState.trim().isEmpty()) { hasError=true;
                errorMessage="Missing wallet state parameter. Please try again." ; walletState="" ; // Set to empty to
                avoid null errors } if (sessionDataKey==null || sessionDataKey.trim().isEmpty()) { hasError=true;
                errorMessage="Missing session data key. Please try again." ; sessionDataKey="" ; // Set to empty to
                avoid null errors } %>
                <!DOCTYPE html>
                <html lang="en">
                ...
                <body>
                    <% if (hasError) { %>
                        <div class="container">
                            ...
                            <div
                                style="background: #f8d7da; color: #721c24; padding: 20px; border-radius: 8px; margin: 20px 0;">
                                <strong>Error:</strong>
                                <%= errorMessage %>
                            </div>
                ...
                <% return; // Stop processing } %>
                    ...
                    <div class="container">
                        ...
                        <div class="qr-section">
                            ...
                            <p id="qrStatus" style="color: #666; font-size: 13px; margin-top: 10px;">Generating QR
                                code...</p>
                            <div class="state-info">
                                Session ID:
                                <code><%= sessionDataKey != null && !sessionDataKey.isEmpty() ? sessionDataKey.substring(0, Math.min(16, sessionDataKey.length())) + "..." : "N/A" %></code>
                            </div>
                        </div>
                            ...
                            pollingInterval = setInterval(() => {
                                ...
                                fetch('/wallet-callback/status?state=' + encodeURIComponent(WALLET_STATE))
                                    ...
                                    .then(data => {
                                        ...
                                        if (data.status === 'received' || data.status === 'VP_SUBMITTED' || data.tokenReceived || data.authenticated) {
                                            clearInterval(pollingInterval);
