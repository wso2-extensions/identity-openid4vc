...
public class SignatureVerifier {
    ...
    public static boolean verifySignature(SignedJWT jwt)
        ...
        try {
            ...
            // x5c path: cert chain embedded in JWT header (EUDI / dc+sd-jwt credentials).
            // Use the leaf cert's public key directly — no network call needed.
            List<com.nimbusds.jose.util.Base64> x5cChain = jwt.getHeader().getX509CertChain();
            if (x5cChain != null && !x5cChain.isEmpty()) {
                try {
                    byte[] leafDer = x5cChain.get(0).decode();
                    CertificateFactory cf = CertificateFactory.getInstance("X.509");
                    X509Certificate leafCert = (X509Certificate) cf.generateCertificate(
                            new ByteArrayInputStream(leafDer));
                    boolean signatureValid = verifyJwtSignature(jwt.getParsedString(),
                            leafCert.getPublicKey(), alg);
                    if (signatureValid) {
     ...
     * Used to build a static trust store from a {@code ISSUER_CERT} configured on a
     * Presentation Definition credential, as an alternative to live JWKS discovery.
     *
     * @param pem PEM string (with or without -----BEGIN CERTIFICATE----- headers)
     * @return The public key embedded in the certificate
     * @throws VerificationException If the PEM cannot be parsed
     */
    public static PublicKey extractPublicKeyFromPem(String pem) throws VerificationException {

        if (StringUtils.isBlank(pem)) {
            throw new VerificationClientException(VerificationErrorCode.INVALID_CREDENTIAL,
                    "Issuer certificate PEM is blank.");
        }
        try {
            String stripped = pem
            ...
            throw new VerificationServerException(VerificationErrorCode.INTERNAL_SERVER_ERROR,
                    "Failed to extract public key from issuer certificate PEM.", e);
        }
    }
