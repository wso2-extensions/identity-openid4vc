...
public final class JwtVerifier implements Verifier {
     ...
     * {@inheritDoc}
     *
     * <p>When {@code trustedIssuerKey} is non-null, the JWT signature is verified directly
     * against that key (cert-based trust), skipping DID/JWKS discovery.</p>
     */
    @Override
    public Map<String, Object> handle(final PresentationSubmission submission,
                                      final int tenantId, final String vpToken,
        ...
        try {
            ...
            boolean signatureValid = SignatureVerifier.verifyJwtSignature(vpToken, trustedIssuerKey, alg);
            if (!signatureValid) {
                throw new VerificationClientException(VerificationErrorCode.INVALID_SIGNATURE,
                        "Signature verification failed for JWT VP against trusted issuer certificate.");
            }
