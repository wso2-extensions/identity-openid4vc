...
CREATE TABLE IF NOT EXISTS IDP_PROV_CONFIG_PROPERTY (
            ...
            IS_SECRET CHAR (1) DEFAULT '0',
            PRIMARY KEY (ID),
            UNIQUE (TENANT_ID, PROVISIONING_TEMPLATE_ID, PROPERTY_KEY),
            FOREIGN KEY (PROVISIONING_TEMPLATE_ID) REFERENCES IDP_PROVISIONING_CONFIG(ID) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS IDP_PROVISIONING_ENTITY (
            ...
            PRIMARY KEY (ID),
            UNIQUE (ENTITY_TYPE, TENANT_ID, ENTITY_LOCAL_USERSTORE, ENTITY_NAME, PROVISIONING_TEMPLATE_ID),
            UNIQUE (PROVISIONING_TEMPLATE_ID, ENTITY_TYPE, ENTITY_VALUE),
            FOREIGN KEY (PROVISIONING_TEMPLATE_ID) REFERENCES IDP_PROVISIONING_CONFIG(ID) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS IDN_REMOTE_FETCH_REVISIONS (
	...
	PRIMARY KEY (ID),
	FOREIGN KEY (TEMPLATE_ID) REFERENCES IDN_REMOTE_FETCH_CONFIG(ID) ON DELETE CASCADE,
	CONSTRAINT UC_REVISIONS UNIQUE (TEMPLATE_ID, ITEM_NAME)
);
...
CREATE INDEX IDX_REMOTE_FETCH_REVISION_TEMPLATE_ID ON IDN_REMOTE_FETCH_REVISIONS (TEMPLATE_ID);
