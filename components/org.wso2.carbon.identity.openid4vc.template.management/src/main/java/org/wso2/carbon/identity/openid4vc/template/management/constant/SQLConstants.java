...
public final class SQLConstants {
    ...
    // IDN_OIDC_VP_TEMPLATE - Presentation Templates
    public static final String CREATE_PRESENTATION_TEMPLATE =
            "INSERT INTO IDN_OIDC_VP_TEMPLATE (ID, TENANT_ID, CLIENT_ID, VERSION, TEMPLATE_JSON, " +
            "IS_PUBLIC, STATUS, CREATED_AT, UPDATED_AT) VALUES (?,?,?,?,?,?,?,?,?)";

    public static final String GET_PRESENTATION_TEMPLATE =
            "SELECT ID, TENANT_ID, CLIENT_ID, VERSION, TEMPLATE_JSON, IS_PUBLIC, STATUS, " +
            "CREATED_AT, UPDATED_AT FROM IDN_OIDC_VP_TEMPLATE " +
            "WHERE CLIENT_ID = ? AND VERSION = ? AND TENANT_ID = ?";

    public static final String UPDATE_PRESENTATION_TEMPLATE =
            "UPDATE IDN_OIDC_VP_TEMPLATE SET TEMPLATE_JSON = ?, IS_PUBLIC = ?, STATUS = ?, " +
            "UPDATED_AT = ? WHERE CLIENT_ID = ? AND VERSION = ? AND TENANT_ID = ?";

    public static final String DELETE_PRESENTATION_TEMPLATE =
            "DELETE FROM IDN_OIDC_VP_TEMPLATE WHERE CLIENT_ID = ? AND VERSION = ? AND TENANT_ID = ?";

    public static final String LIST_PRESENTATION_TEMPLATES =
            "SELECT ID, TENANT_ID, CLIENT_ID, VERSION, TEMPLATE_JSON, IS_PUBLIC, STATUS, " +
            "CREATED_AT, UPDATED_AT FROM IDN_OIDC_VP_TEMPLATE WHERE TENANT_ID = ?";

    public static final String EXISTS_PRESENTATION_TEMPLATE =
            "SELECT 1 FROM IDN_OIDC_VP_TEMPLATE WHERE CLIENT_ID = ? AND VERSION = ? AND TENANT_ID = ?";
}
